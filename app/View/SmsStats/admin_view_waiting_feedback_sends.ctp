<div class="edmStats index sms">
	<div class="min-height">
		<div class="title"><small><?php echo __('Campaign'); ?>:</small> <?php echo $this->Text->truncate(h($campaign['Campaign']['name']), 75, array('exact' => false)); ?>
			<div class="total"><?php echo __('Waiting Feedback/Other: %s', number_format($total_sms_stats)); ?></div>
		</div>
		<?php
		if (!empty($sms_stats)) {
			echo $this->element('admin/export_breakdowns', array(
				'controller' => 'sms_stats',
				'action' => 'get_waiting_feedback_sends_export_status',
				'form_action' => 'export_waiting_feedback_sends',
				'export_name' =>  __('Waiting Feedback'),
				'export_type' => 'sms_waiting_feedback_sent',
		...
		<table cellpadding="0" cellspacing="0" class="popup-table table table-striped">
			<thead>
				<tr>
					...
					<th width="560" class="text-center"><?php echo __('Description');?></th>
			...
			<tbody>
				...
				<tr>
					...
					if ($globalconf['GL.useMemberId'] == 1) {
					?>
					<td align="center"><?php echo $member_id; ?>&nbsp;</td>
					<?php
					}
					?>
					<?php
					...
					<td><?php echo __("Message has been put on queue or already passed to gateway, now is waiting carrier's feedback, if any."); ?>&nbsp;</td>
	...
	echo $this->element('admin/solr_index_paging', array(
		...
		'current_page' => $page,
		'url' => $this->Html->url(array('client' => $client_db_name, 'action' => 'view_waiting_feedback_sends', 'campaign_id' => $campaign_id, 'batch_id' => $batch_id, 'mobile' => $search_mobile))
	));
	?>
</div>
