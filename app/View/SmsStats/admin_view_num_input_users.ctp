<div class="edmStats index sms">
	<div class="min-height">
		<div class="title"><small><?php echo __('Campaign'); ?>:</small> <?php echo $this->Text->truncate(h($campaign['Campaign']['name']), 75, array('exact' => false)); ?>
			<div class="total"><?php echo __('Input Mobile: %s', number_format($total_sms_stats)); ?></div>
		</div>
		<?php
		if (!empty($sms_stats)) {
			echo $this->element('admin/export_breakdowns', array(
				'controller' => 'sms_stats',
				'action' => 'get_sms_num_input_users_export_status',
				'form_action' => 'export_num_input_users',
				'export_name' =>  __('Input Mobile'),
				'export_type' => 'sms_num_input_users',
	...
	echo $this->element('admin/solr_index_paging', array(
		...
		'current_page' => $page,
		'url' => $this->Html->url(array('client' => $client_db_name, 'action' => 'view_num_input_users', 'campaign_id' => $campaign_id, 'batch_id' => $batch_id, 'mobile' => $search_mobile))
	));
	?>
</div>
