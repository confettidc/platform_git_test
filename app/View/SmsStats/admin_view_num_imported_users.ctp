<div class="edmStats index sms">
	<div class="min-height">
		<div class="title"><small><?php echo __('Campaign'); ?>:</small> <?php echo $this->Text->truncate(h($campaign['Campaign']['name']), 75, array('exact' => false)); ?>
			<div class="total"><?php echo __('Imported Mobile: %s', number_format($total_sms_stats)); ?></div>
		</div>
		<?php
		if (!empty($sms_stats)) {
			echo $this->element('admin/export_breakdowns', array(
				'controller' => 'sms_stats',
				'action' => 'get_sms_num_imported_users_export_status',
				'form_action' => 'export_num_imported_users',
				'export_name' =>  __('Imported Mobile'),
				'export_type' => 'sms_num_imported_users',
		...
		<table cellpadding="0" cellspacing="0" class="popup-table table table-striped">
				...
				<tr>
					...
					<th class="text-left">
						...
						if ($campaign['Campaign']['sms_merge'] == 1) {
							echo __('Content after SMS Merge');
						} else {
							echo __('Content');
						}
						?>
					</th>
					<th width="80" class="text-center"><?php echo __('Delivery Datetime');?></th>
					<th width="80" class="text-center"><?php echo __('Word Count');?></th>
					<th width="100" class="text-center"><a class="tooltip" onmouseout="hideddrivetip()" ;="" onmouseover="ddrivetip('<?php echo __('Original SMS Count &lt;span style=&quot;color: red;&quot;&gt;(+Additional)&lt;/span&gt; &lt;span style=&quot;color: green;&quot;&gt;(-Refund)&lt;/span&gt;'); ?>', 220)"><img alt="tooltip" src="/img/icon-tooltip-right.gif" /></a><?php
						if ($campaign['Campaign']['sms_merge'] == 1) {
					...
					?></th>
					<th width="200" class="text-center"><?php echo __('Delivery Status');?></th>
			...
			<tbody>
				...
				<tr>
					...
					?>
					<td class="text-center"><?php echo empty($country_code) ? "&lt;" . __('Empty Country Code') . "&gt;" : $country_code; ?>&nbsp;</td>
					...
					<td>
						<div class="sms-content">
							...
							<div class="wrap" id="wrap_<?php echo $sms_stat_id; ?>">
								 <div>
									  <?php echo nl2br(h($sms_stat_contents[$sms_stat_id])); ?>
								 </div>
					...
					<td class="text-center">
						<?php
						echo number_format($word_count_arr[$sms_stat_id]);
						?>
					</td>
					<td class="text-center">
						<?php
						echo number_format($sms_count_arr[$sms_stat_id]);
						?>
					</td>
	...
	echo $this->element('admin/solr_index_paging', array(
		...
		'current_page' => $page,
		'url' => $this->Html->url(array('client' => $client_db_name, 'action' => 'view_num_imported_users', 'campaign_id' => $campaign_id, 'batch_id' => $batch_id, 'mobile' => $search_mobile))
	));
	?>
</div>
