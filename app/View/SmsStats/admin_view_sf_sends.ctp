<div class="edmStats index sms">
	<div class="min-height">
		<div class="title"><small><?php echo __('Campaign'); ?>:</small> <?php echo $this->Text->truncate(h($campaign['Campaign']['name']), 75, array('exact' => false)); ?>
			<div class="total"><?php echo __('Failure: %s', number_format($total_sms_stats)); ?></div>
		</div>
		<?php
		if (!empty($sms_stats)) {
			echo $this->element('admin/export_breakdowns', array(
				'controller' => 'sms_stats',
				'action' => 'get_sf_sends_export_status',
				'form_action' => 'export_sf_sends',
				'export_name' =>  __('Failure Sent'),
				'export_type' => 'sms_sf_sent',
		...
		<table cellpadding="0" cellspacing="0" class="popup-table table table-striped">
			<thead>
				<tr>
					...
					<th width="120" class="text-center"><?php echo __('Mobile');?></th>
					<th width="100" class="text-center"><?php echo __('Delivery Datetime');?></th>
					<th width="540" class="text-left"><?php echo __('Description');?></th>
			...
			<tbody>
				...
				foreach ($sms_stats as $sms_stat) {
					...
					if ($globalconf['GL.useMemberId'] == 1) {
						if (strpos($sms_stat['mobile'], '||') !== false) {
							list($member_id, $country_code_mobile) = explode('||', $sms_stat['mobile'], 2);
						} else {
							$member_id = '';
				...
				<tr>
					...
					<td><?php echo $this->CodeDesc->getSmsFailureStatDesc($sms_stat); ?>&nbsp;</td>
	...
	echo $this->element('admin/solr_index_paging', array(
		...
		'current_page' => $page,
		'url' => $this->Html->url(array('client' => $client_db_name, 'action' => 'view_sf_sends', 'campaign_id' => $campaign_id, 'batch_id' => $batch_id, 'mobile' => $search_mobile))
	));
	?>
</div>
