<div class="edmStats index sms">
	<div class="min-height">
		<div class="title"><small><?php echo __('Campaign'); ?>:</small> <?php echo $this->Text->truncate(h($campaign['Campaign']['name']), 75, array('exact' => false)); ?>
			<div class="total"><?php echo __('Credit Used: %s%s', $currency_display, number_format($sms_used_credits, 3)); ?></div>
		</div>
		<?php
		if (!empty($sms_stats)) {
			echo $this->element('admin/export_breakdowns', array(
				'controller' => 'sms_stats',
				'action' => 'get_sms_num_final_sms_sends_export_status',
				'form_action' => 'export_num_final_sms_sends',
				'export_name' =>  __('Credit Used'),
				'export_type' => 'sms_num_final_sms_sent',
		...
		<table cellpadding="0" cellspacing="0" class="popup-table table table-striped">
			<thead>
				<tr>
					...
					<th width="100"><a class="tooltip" onmouseout="hideddrivetip()" ;="" onmouseover="ddrivetip('<?php echo __('Original SMS Count &lt;span style=&quot;color: red;&quot;&gt;(+Additional)&lt;/span&gt; &lt;span style=&quot;color: green;&quot;&gt;(-Refund)&lt;/span&gt;'); ?>', 220)"><img alt="tooltip" src="/img/icon-tooltip-right.gif" /></a><?php
						if ($campaign['Campaign']['sms_merge'] == 1) {
					...
					?></th>
					<th class="text-center"><?php echo __('Unit Price');?></th>
					<th class="text-center"><?php echo __('Total Price');?></th>
				</tr>
			</thead>
			<tbody>
				<?php
	...
	echo $this->element('admin/solr_index_paging', array(
		...
		'current_page' => $page,
		'url' => $this->Html->url(array('client' => $client_db_name, 'action' => 'view_num_final_sms_sends', 'campaign_id' => $campaign_id, 'batch_id' => $batch_id, 'mobile' => $search_mobile))
	));
	?>
</div>
